...
async function collectMdFilesAll(dir: string): Promise<string[]> {
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      ...
      // Include .clsc.md — used as anchors for ARCHIVE orphans (must have Obsidian wikilinks)
      results.push(fullPath);
    }
  }
  return results;
...
// .clsc.md files now have proper Obsidian headers with wikilinks so they ARE graph nodes.
function findClscAnchorInParents(startDir: string, vaultRoot: string, allMdFilesSet: Set<string>): string | null {
  ...
  while (dir !== vaultNorm) {
    ...
    // Also check for any .clsc.md at this level (e.g. chats.clsc.md)
...
// Priority: exact-name .clsc.md → _index.clsc.md in same dir → walk parents.
...
export async function runOrphanScanner(
  ...
  for (const orphan of orphans) {
    ...
    if (type === "CODE") {
      ...
      // Link via .clsc.md anchor — .clsc.md files now have proper Obsidian headers + wikilinks.
      // Priority: exact-name .clsc.md → _index.clsc.md in dir → walk parents.
