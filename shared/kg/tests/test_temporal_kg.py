...
def test_temporal_as_of():
    ...
    try:
        ...
        # Add: Wes was shareholder from 2024-01-01
        kg.add_triple(
            subject="Wes",
            predicate="role",
            obj="shareholder",
        ...
        past_facts = kg.query_entity("Wes", as_of="2024-06-01")
        ...
        today_facts = kg.query_entity("Wes", as_of="2026-04-08")
...
def test_demo_facts_backfill():
    ...
    wes_facts = kg_query("Wes", direction="outgoing")
    assert len(wes_facts) > 0, "No facts found for Wes"
    print(f"PASS test_demo_facts_backfill ({len(wes_facts)} Wes facts active today)")

    # Also verify 老兔 CEO fact exists
    laotu_facts = kg_query("老兔", direction="outgoing")
    laotu_roles = [f.get("object") for f in laotu_facts]
    assert "CEO" in laotu_roles, f"Expected CEO for 老兔, got: {laotu_roles}"
    print(f"PASS test_demo_facts_backfill (老兔 roles: {laotu_roles})")
...
def test_query_all():
    ...
    # as_of 2023-01-01 should return 0 (no facts before 2020 except 老兔 CEO)
