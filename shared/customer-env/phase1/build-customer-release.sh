...
bun "$here/generate-env-manifest.ts" "$source_root" "$here/customer-source-allowlist.txt" \
...
bun build "$source_root/mvp-server.ts" --target=bun --minify --outfile="$output/release/dist/mvp-server.js"
