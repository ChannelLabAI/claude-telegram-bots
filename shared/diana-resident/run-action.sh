...
audit() { printf '%s action=%s result=%s reason=%s\n' "$stamp" "$action" "$1" "$2" >> "$audit_dir/actions.log"; }
...
case "$action" in
 ...
 terminate_verified_orphan)
   pid=${1:-}; proc_root=${DIANA_PROC_ROOT:-/proc}; cgroup_root=${DIANA_CGROUP_ROOT:-/sys/fs/cgroup}; activity_root=${DIANA_ORPHAN_ACTIVITY_ROOT:-/run/user/1002/diana-orphan-activity}; now=${DIANA_NOW_EPOCH:-$(date +%s)}; idle_secs=${DIANA_ORPHAN_IDLE_SECS:-900}
   [[ "$pid" =~ ^[1-9][0-9]*$ && "$idle_secs" =~ ^[0-9]+$ && -r "$proc_root/$pid/status" && -r "$proc_root/$pid/cgroup" ]] || { audit DENY role_not_verified; exit 2; }
