...
ChannelLab Knowledge Base MCP server — a local-only library-mode server that wraps FTS5 message search, closet content retrieval, temporal knowledge graph queries, learned skill access, and task queue creation into a single Claude Code MCP integration.

## Install

```bash
pip install -e ~/.claude-bots/shared/channellab-kb-mcp
claude mcp add channellab python -m channellab_kb_mcp
```
...
```bash
python3 -m venv ~/.venvs/channellab-mcp
source ~/.venvs/channellab-mcp/bin/activate
pip install -e /path/to/channellab-kb-mcp
```

Then register with the venv Python:

```bash
claude mcp add channellab ~/.venvs/channellab-mcp/bin/python -m channellab_kb_mcp
```
