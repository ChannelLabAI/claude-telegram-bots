"""
server.py — ChannelLab Knowledge Base MCP Server (library mode, local only).

Registered tools:
  channellab_fts_search    — FTS5 cross-bot message search
  channellab_closet_get    — Closet content retrieval (verbatim/skeleton)
  channellab_closet_search — Multi-term AND search over CLSC skeleton closet
  channellab_kg_query      — Temporal knowledge graph query
  channellab_skill_list    — List/get approved learned skills
  channellab_task_create   — Create task in pending queue

Run via:
  python -m channellab_kb_mcp
  channellab-kb-mcp
"""
import json
import logging
...
logger = logging.getLogger("channellab_kb_mcp")
...
TOOLS = {
    ...
    "channellab_closet_get": {
        "description": (
            "Retrieve content from ChannelLab's closet knowledge store by slug. "
    ...
    "channellab_closet_search": {
        "description": (
            "Search ChannelLab's CLSC skeleton closet using multi-term AND matching. "
    ...
    "channellab_task_create": {
        "description": (
            "Create a new task in the ChannelLab task queue (pending/). "
    ...
    if method == "initialize":
        return {
            ...
            "result": {
                ...
                "capabilities": {"tools": {}},
                "serverInfo": {"name": "channellab-kb-mcp", "version": "0.1.0"},
            },
        }
...
# Module-level alias so `from channellab_kb_mcp.server import mcp` works.
...
def main():
    logger.info("ChannelLab KB MCP Server starting (library mode)...")
    while True:
        try:
