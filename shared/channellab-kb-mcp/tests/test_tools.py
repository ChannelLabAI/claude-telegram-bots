"""
test_tools.py — Integration tests for channellab-kb-mcp tools.
...
from channellab_kb_mcp.config import FTS_DB, KG_DB, TASKS_ROOT, LEARNED_SKILLS_DIR, SHARED_ROOT
...
def test_fts_search_basic():
    ...
    from channellab_kb_mcp.tools.fts_search import fts_search
...
def test_fts_search_missing_db(tmp_path, monkeypatch):
    ...
    import channellab_kb_mcp.config as config_mod
    monkeypatch.setattr(config_mod, "FTS_DB", tmp_path / "nonexistent.db")

    # Re-import to pick up patched config
    import importlib
    import channellab_kb_mcp.tools.fts_search as fts_mod
...
def test_kg_query_laotu():
    ...
    from channellab_kb_mcp.tools.kg_query import kg_query
...
def test_closet_get_stub_verbatim():
    ...
    from channellab_kb_mcp.tools.closet_get import verbatim_fetch
...
def test_closet_get_mode_verbatim():
    ...
    from channellab_kb_mcp.tools.closet_get import closet_get
...
def test_skill_list_returns_list():
    ...
    from channellab_kb_mcp.tools.skill_tools import skill_list
...
def test_skill_get_known():
    ...
    from channellab_kb_mcp.tools.skill_tools import skill_list, skill_get
...
def test_skill_get_missing():
    ...
    from channellab_kb_mcp.tools.skill_tools import skill_get
...
def test_task_create_dry_run(tmp_path, monkeypatch):
    ...
    import importlib
    import channellab_kb_mcp.tools.task_create as tc_mod
...
def test_task_create_real_pending(tmp_path, monkeypatch):
    ...
    from channellab_kb_mcp.tools.task_create import task_create
...
def test_server_import():
    ...
    from channellab_kb_mcp.server import TOOLS, handle_request

    assert len(TOOLS) == 6
    expected = {
        "channellab_fts_search",
        "channellab_closet_get",
        "channellab_closet_search",
        "channellab_kg_query",
        "channellab_skill_list",
        "channellab_task_create",
    }
    assert set(TOOLS.keys()) == expected


def test_server_tools_list():
    """handle_request('tools/list') returns all 5 tools."""
    from channellab_kb_mcp.server import handle_request
    ...
    assert "channellab_fts_search" in names
    assert "channellab_task_create" in names
...
def test_server_initialize():
    ...
    assert response["result"]["serverInfo"]["name"] == "channellab-kb-mcp"
...
def test_closet_get_path_traversal_blocked():
    ...
    import pytest
    from channellab_kb_mcp.tools.closet_get import verbatim_fetch, skeleton_read
...
def test_closet_search_single_term():
    ...
    from channellab_kb_mcp.tools.closet_search import closet_search
...
def test_closet_search_via_server():
    ...
    from channellab_kb_mcp.server import TOOLS
    assert "channellab_closet_search" in TOOLS
    spec = TOOLS["channellab_closet_search"]
