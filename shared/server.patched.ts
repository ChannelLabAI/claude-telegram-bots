 ...
 * Modifications Copyright 2026 ChannelLabAI.
 *
...
// Prevent EPIPE / stdout errors from crashing the process
process.stdout.on('error', (err: Error) => {
  log(`stdout error: ${err.message}`)
})

process.stdin.on('end', () => {
  log('stdin ended — harness closed pipe, shutting down bot')
  shutdown()
})
...
if (!STATIC) approvalTimer = setInterval(checkApprovals, 5000)
  ...
  try {
    switch (req.params.name) {
      case 'reply': {
        ...
        // Relay to other bots if this is a group chat
        if (chat_id.startsWith('-') && botUsername) {
          writeRelay(chat_id, text, sentIds[sentIds.length - 1])
        }
...
// Graceful shutdown — stop bot polling and close MCP transport.
...
bot.start({
  ...
  process.stderr.write(`telegram channel: bot polling failed: ${err}\n`)
  process.exit(1)
})
